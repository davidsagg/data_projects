...
def init_postgres():
    ...
    db_connection = conectar_postgres()
    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS chat_history (
                ...
                category VARCHAR(50) DEFAULT 'data_science',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        db_connection.commit()
        cursor.close()
        logger.info("Tabela chat_history criada/verificada no PostgreSQL")
        return True
    except Exception as e:
...
def save_to_postgres(query: str, response: str, category: str):
    if not db_connection:
        logger.warning("Conexão PostgreSQL não disponível")
        return False
    
    try:
        cursor = db_connection.cursor()
        cursor.execute(
            "INSERT INTO chat_history (query, response, category) VALUES (%s, %s, %s)",
...
def get_history_from_postgres(limit: int = 10, category: str = None):
    if not db_connection:
        logger.warning("Conexão PostgreSQL não disponível")
        return []
...
RESPOSTA:"""
        ...
        save_to_postgres(request.query, response_text, category)
...
async def get_history(limit: int = 10, category: str = None):
    history = get_history_from_postgres(limit=limit, category=category)
